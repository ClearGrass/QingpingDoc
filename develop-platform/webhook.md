...
- 推送的数据位于 HTTP/HTTPS 请求的 Body 中，均为 Json 结构，头部类型为 "Content-Type: application/json"；
- HTTP/HTTPS 请求发出后，请在 3 秒内进行回应，否则平台认为请求超时，该次请求进入延迟推送状态；
- HTTP/HTTPS 请求返回码为 200 时，表示数据推送成功，返回其他状态码，平台判断为推送失败，该次请求进入延迟推送状态；
- 延迟推送会分别在 5、15、30 分钟后尝试重新发起 HTTP/HTTPS 请求进行数据推送，如果期间有成功推送或 3 次均失败，都会终止该次数据后续推送，即丢弃该任务（如果仍然希望获取该丢失数据，可使用开放接口进行获取）
...
设备数据子类描述
***注意：*** 不同传感器种类输出的数据格式不同，所以该结构里的字段部分可选，具体参考 [规范说明 - 2. 设备列表及属性支持说明](/main/specification#2-设备列表及属性支持说明)
...
| status       | int    | C        | 事件发生或解除（1-触发，2-解除）                |
